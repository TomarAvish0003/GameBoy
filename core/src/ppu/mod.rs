...
const LCDC_BG_WNDW_TITLE_BIT: u8 = 4;
...
impl Ppu {
    pub fn new() -> Self {
        Self {
            screen_buffer: [0xFF; DISPLAY_BUFFER],
            mode: Lcd::new(),
    ...
    fn get_bg_wndw_tile_set_index(&self) -> u8 {
        let lcdc = self.read_lcd_reg(LCDC);
        if lcdc.get_bit(LCDC_BG_WNDW_TITLE_BIT) { 1 } else { 0 }
    }

    fn get_bg_tile_index(&self) -> u8 {
        let lcdc = self.read_lcd_reg(LCDC);
    ...
    pub fn render_scanline(&mut self) {
        ...
        let mut row = [0xFF; (SCREEN_WIDTH as usize) * 4];
        ...
        let start_idx = line as usize * (SCREEN_WIDTH as usize) * 4;
        let end_idx = (line as usize + 1) * (SCREEN_WIDTH as usize) * 4;
        self.screen_buffer[start_idx..end_idx].copy_from_slice(&row);
    }

    fn render_bg(&self, buffer: &mut [u8], line: u8) {
        let map_offset = self.get_bg_tile_index() as usize * TILE_MAP_TABLE_SIZE;
    ...
    fn render_window(&self, buffer: &mut [u8], line: u8) {
        ...
        let row = y % TILESIZE;
        for x in (coords.x as usize)..(SCREEN_WIDTH as usize) {
            let col = x % TILESIZE;
            let map_num = (y / TILESIZE) * LAYERSIZE + (x / TILESIZE);
            let tile_index = self.maps[map_offset + map_num] as usize;
            let adjusted_tile_index = if self.get_bg_wndw_tile_set_index() == 1 {
    ...
    fn render_sprites(&self, buffer: &mut [u8], line: u8) {
        ...
        for spr in sprites {
            let height = if is_8x16 { 16isize } else { 8isize };
            let coords = spr.get_coords();
            let signed_line = line as isize;
            if signed_line < coords.1 || coords.1 + height <= signed_line {
                continue;
            }
            let palette = self.get_sprite_palette(if spr.use_palette1() { 1 } else { 0 });
